...
let appState = {
    ...
    // --- NEW ---: State to hold active chat user info
    activeChat: {
        userId: null,
        username: '',
        avatar: ''
...
// Elements for the Premium Upload Screen
...
// --- NEW ---: Category selector for premium screen
const premiumCategorySelector = document.getElementById('premium-category-selector');
...
function activateScreen(screenId) {
    screens.forEach(screen => {
        ...
        // Ensure scrollability on screens that need it
        if (isActive && (screenId === 'information-screen' || screenId === 'premium-upload-screen')) {
            screen.style.overflowY = 'auto';
        } else if (isActive) {
            screen.style.overflowY = 'hidden'; // Default for most screens
        }
    });
    // Special handling for home screen's inner swiper
    if (screenId === 'home-screen') {
        document.getElementById('video-swiper').style.overflowY = 'scroll';
    }
...
function navigateTo(nextScreenId) {
    ...
    // --- MODIFIED ---: Call render functions based on navigation
    if (nextScreenId === 'profile-screen') loadUserVideosFromFirebase();
    if (nextScreenId === 'image-editor-screen') photoEditor.start();
    if (nextScreenId === 'chat-screen') renderChatScreen(); // For the new chat screen
}
...
function toggleCategoryOptions(selectorType = 'modal') {
    let displayElement;
    ...
    if(displayElement) displayElement.classList.toggle('open');
}

function selectCategory(category, selectorType = 'modal') {
    appState.uploadDetails.category = category;
    let textElement, displayElement;
    if (selectorType === 'modal') {
        textElement = document.getElementById('selected-category-text');
        displayElement = document.querySelector('#upload-details-modal .category-selector-display');
    } else { // premium
        textElement = document.getElementById('premium-selected-category-text');
        displayElement = document.querySelector('#premium-upload-screen .category-selector-display');
    }
    
    if(textElement) textElement.textContent = category;
    if(displayElement) displayElement.classList.remove('open');
}
...
function renderCategories() {
    // For YouTube Modal
    categoryOptionsContainer.innerHTML = categories.map(cat => `<div class="category-option" onclick="selectCategory('${cat}', 'modal')">${cat}</div>`).join('');
    
    // --- NEW ---: For Premium Upload Screen
    const premiumCategoryOptionsContainer = document.getElementById('premium-category-options');
    if (premiumCategoryOptionsContainer) {
        premiumCategoryOptionsContainer.innerHTML = categories.map(cat => `<div class="category-option" onclick="selectCategory('${cat}', 'premium')">${cat}</div>`).join('');
    }
}
        ...
        if (playerElement && !players[video.id]) {
            if (video.videoType === 'premium') {
                ...
                players[video.id] = new YT.Player(playerId, {
                    height: '100%', width: '100%', videoId: video.videoUrl,
    ...
    videoObserver = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
            ...
            if (entry.isIntersecting) {
                if (activePlayerId && activePlayerId !== videoId) pauseActivePlayer(activePlayerId);
...
// =======================================================================
// === MODIFIED: Friends/Chat Screen Functions ===
// =======================================================================

function openChatWindow(userId, username, avatar) {
    // --- MODIFIED ---: Instead of an alert, we navigate to the chat screen
    appState.activeChat = { userId, username, avatar };
    navigateTo('chat-screen');
}

function renderChatScreen() {
    // This function will populate the chat screen UI.
    // NOTE: You need to add the 'chat-screen' div to your HTML file.
    const { username, avatar } = appState.activeChat;
    ...
    if (chatHeaderAvatar) chatHeaderAvatar.src = avatar || 'https://via.placeholder.com/40';

    // Placeholder: You would fetch actual chat messages here.
    if (chatMessagesList) {
        chatMessagesList.innerHTML = `
            <div class="chat-message received">
                <p>Hey, how are you? Let's catch up tomorrow.</p>
...
function showFriendsSubView(viewName) {
    // This can be expanded to show different content in the friends screen
...
function handlePremiumFileUpload() {
    ...
    if (!file || !title || !category) {
        alert("Please select a video file, enter a title, and choose a category.");
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (openYouTubeBtn) openYouTubeBtn.addEventListener('click', openUploadDetailsModal);
    
    const openPremiumBtn = document.getElementById('open-premium-upload-btn');
    if (openPremiumBtn) openPremiumBtn.addEventListener('click', () => navigateTo('premium-upload-screen'));
    ...
    // Event listeners for the premium upload screen
    if (premiumUploadBtn) premiumUploadBtn.addEventListener('click', handlePremiumFileUpload);
    if (backFromPremiumBtn) backFromPremiumBtn.addEventListener('click', () => navigateTo('upload-screen'));
    ...
    // Event Listeners for Friends Screen
    ...
    // ... add more for status, story etc.

    // --- NEW ---: Event listener for the new photo editor sidebar toggle
    const toggleSidebarBtn = document.getElementById('toggle-sidebar-btn');
    const editorSidebar = document.getElementById('editor-sidebar');
    if (toggleSidebarBtn && editorSidebar) {
        toggleSidebarBtn.addEventListener('click', () => {
            editorSidebar.classList.toggle('visible');
        });
    }
});
...
const photoEditor = (() => {
    ...
    function initializeState() { /* State initialization... (same as before) */ }
    function initialize() {
        mainImage = getEl('mainImage'); imageWrapper = getEl('image-wrapper'); uploadInput = getEl('upload'); cameraBtn = getEl('camera-btn'); videoFeed = getEl('video-feed'); captureBtn = getEl('capture-btn');
        // --- MODIFIED --- Use the new sidebar and bottom panel
        toolbarNav = getEl('editor-sidebar'); 
        panelContainer = getEl('editor-bottom-panel'); 
        ...
        // --- MODIFIED --- Click the filter button in the new sidebar
        document.querySelector('#image-editor-screen .tool-btn[data-tool="filters"]').click();
    ...
    // ... all other photo editor functions (populateFilters, setupEventListeners, etc.) remain unchanged ...
    // The following functions are exactly the same as in your provided code.
    ...
    function setupEventListeners() {uploadInput.addEventListener('change', handleUpload); cameraBtn.addEventListener('click', toggleCamera); captureBtn.addEventListener('click', captureImage); toolbarNav.addEventListener('click', handleToolbarClick); fontSelectBtn.addEventListener('click', () => fontPickerModal.style.display = 'flex');fontPickerModal.addEventListener('click', (e) => { if (e.target === fontPickerModal || e.target.closest('.font-option')) { const r = e.target.closest('.font-option')?.querySelector('input[type="radio"]'); if (r) { currentFont = fonts[r.value]; fontSelectBtn.textContent = currentFont.name; fontSelectBtn.style.fontFamily = currentFont.value; } fontPickerModal.style.display = 'none'; } });document.querySelectorAll('#image-editor-screen .adjust-slider-input').forEach(s => s.addEventListener('input', handleAdjustSlider));applyCropBtn.addEventListener('click', applyCrop); cancelCropBtn.addEventListener('click', endCrop); cropShapesContainer.addEventListener('click', handleCropShapeChange);}
